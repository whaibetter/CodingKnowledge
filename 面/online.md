...
AOP（Aspect-Oriented Programming）和 IOC（Inversion of Control）是两个常见的软件开发概念，用于**提高代码的模块化、可维护性和可扩展性**。

- aop使得与核心业务代码不相干的代码能够分离开，并且有效降低耦合。**从核心业务逻辑中分离出来，实现了关注点的集中管理和复用。**
- ioc 依赖关系由系统管理，不用手动管理。

AOP 是一种编程范式，用于将**系统的关注点**进行**解耦**，将交叉关注点从核心业务逻辑中分离出来。它通过在特定的切入点处插入横切关注点，实现对系统行为的增强和控制。AOP 的实现方式包括代理模式和动态字节码生成。

IOC 是一种设计原则，用于实现控制反转。它将对**象的创建和组装由外部容器负责**，通过**依赖注入**的方式将依赖关系注入到对象中。这样，对象只需关注自身的职责，而不需要关心依赖的创建和管理。IOC 提高了代码的**可测试性、可维护性和可扩展性**。
