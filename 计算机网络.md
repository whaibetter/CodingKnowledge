...
如果客户端在TIME_WAIT状态下立刻重启并尝试与同一服务器建立新连接，客户端会立即发送SYN包。由于服务器端可能还在处理前一个连接的残留数据，这可能会导致新连接的建立出现延迟或失败。为了避免这种情况，客户端通常会等待一段时间再尝试建立新连接，或者操作系统会自动跳过TIME_WAIT状态立即建立新连接，但这可能会带来潜在的风险。
